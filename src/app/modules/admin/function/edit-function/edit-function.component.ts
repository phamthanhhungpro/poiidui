...
export class EditFunctionComponent {
  ...
  editFunctionForm: UntypedFormGroup;
  ...
  constructor(private _formBuilder: UntypedFormBuilder,
    ...
    this.editFunctionForm = this._formBuilder.group({
  ...
  ngOnInit(): void {
    this.editFunctionForm.patchValue(this.data);
  }

  // clear form when close drawer
  clearForm(): void {
    this.editFunctionForm.reset();
  }
  ...
  // save data
  save(): void {
    this._functionService.update(this.data.id, this.editFunctionForm.value).subscribe(res => {
      if (res.isSucceeded) {
